...
## 0. 最上位の原則：完全自動化（2026-09-02 林さん明示・再三の指示を正本化）

**このダッシュボードは完全自動化が原則。人手の運用（手入力・手貼り・目視転記・コピペ）を前提にした案を出してはいけない。**

- データが足りないときの答えは「人が入れる」ではなく **「取り込む経路を作る」**。
- すでに人手が残っている箇所は**仕様ではなく穴**として扱い、自動化の候補として書き出す。
- 「移植スクリプトが無い」＝「手入力が仕様」ではない。**無いものを見たら第一仮説は "未実装の穴"**。仕様だと断定するなら決定の記録（commit・台帳・docs）を1件示す。示せないなら「記録が見つかりません」と書く。
- 人の判断が要るのは**誤マッチの承認だけ**（例 `server/nameSuggest.ts` は自動確定しない）。それ以外に人の手を挟まない。
- この原則は repo の実装史そのもの：`12c8f72`（ハードコード削除→報告書提出で自動掲載）／`b52374a`（毎月6日自動同期）／`0c29859`（新店舗の自動検出・自動登録）／`56d11cf`（手書き表の廃止）／`e8d108f`（未マッチ名の候補自動推定）。**人手を増やす提案はこの歴史に逆行する。**

> 2026-09-02 の違反例（再発防止のため残す）：ニックネームが画面に出ない件で、はやしんが「Notion に36人手入力」を案として出した。実際は月末報告書 列20（2026-08-29 以降）に13人ぶんの実データが既に存在し、報告書が来るたび自動で増える。**測れば5分で否定できたことを、測らずに人手案として出した。**
